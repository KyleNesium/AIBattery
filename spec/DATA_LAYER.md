...
Stored (pre-computed at construction): `totalTokens: Int` (sum of all `modelTokens.totalTokens`), `totalProjectTokens: Int` (sum of all `projectTokens.totalTokens`), `totalProjectCost: Double` (sum of all `projectTokens.estimatedCost`), `dailyAverage: Int` (average messages/day from last 7 days of `dailyActivity`), `trendDirection: TrendDirection` (requires 14+ days of activity for a symmetric 7-vs-7 comparison; ±10% threshold → `.up`/`.down`/`.flat`), `busiestDayOfWeek: (name: String, averageCount: Int)?` (highest average from `dailyActivity` by weekday).
