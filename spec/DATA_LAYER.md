...
- `isUsingLocalEstimate` and the `percent(for:)` local-estimate fallback are **Claude-only** — `LocalUsageEstimate` / `PlanTier` are Anthropic concepts and a Codex snapshot (`provider == .codex`) never falls back to them (0% until real Codex data arrives).
...
Stored: `rateLimitsFresh: Bool` — whether the rate-limit *values* came from a genuinely fresh fetch this cycle (unified headers AND not cache-served). Set by `UsageAggregator.aggregate(rateLimitsFresh:)` from `UsageViewModel.rateLimitsFresh` as written in `refresh()`. Part of the hand-written `==` (a fresh↔stale flip changes the displayed %, so it must re-render).

Computed: `rateLimitPercentConfirmed(for window:) -> Bool` — **per-window** *alarm* gate = `rateLimitsFresh || rateLimits.isWindowThrottled(window)`. Scoped per window so a genuine throttle on one window (authoritative, carries a reset, self-expires) can't confirm a *stale* near-full reading on the other. Gates **only the alarm** (passed to the popover bars as their `confirmed` gate — the red "Limit reached" / broken star); the displayed percentage is always the real API value (`percent(for:)` does no confirmed-based substitution).

Computed: `percent(for: MetricMode) -> Double` (shared metric percentage calculation used by both menu bar and popover) always shows the **real API utilization** for rate-limit modes when rate limits are present — `rateLimits?.fiveHourPercent`/`sevenDayPercent`, fresh or the last-known held value — falling back to `LocalUsageEstimate.fiveHourPercent`/`sevenDayPercent` **only when there is no API data at all** (`rateLimits == nil`, standard-API-key users). A token-derived estimate is never substituted for a held API reading (a guess doesn't make sense once the API has spoken); a fresh-but-wrong near-full reading is corrected *upstream* in `refresh()` by the spike filter (`spikeConfirmedRateLimits`, which holds it at the previous real value), so `percent(for:)` needs no freshness-based substitution. Freshness gates only the alarm (`rateLimitPercentConfirmed(for:)`, below). Context health uses `topSessionHealths.first?.usagePercentage` as primary, falls back to `tokenHealth?.usagePercentage`, so auto mode reflects the most critical session, not just the most recent), `isUsingLocalEstimate: Bool` (true when no API rate limits and local estimate can provide percentages — drives conditional rendering of `LocalEstimateSection`), `autoResolvedMode: MetricMode` (four-tier deterministic escalation ladder: **Tier 1** — if throttled, always shows the binding rate limit window; **Tier 2** — if either rate limit >= 80%, shows the higher rate limit window; **Tier 3** — if any active session has context >= 60%, shows context health; **Tier 4** — default shows binding (highest-consumed) rate limit. Used by `UsageViewModel` to derive the candidate for hysteresis filtering), `hasActiveSession: Bool` (whether any tracked session has been active within the staleness window — used by `autoResolvedMode` Tier 3 and `applyHysteresis` context hold check).
...
- `shortLabel(provider:creditBudget:)` — "7 Day" (Claude) / "Weekly" (Codex windowed) / "Credits" (Codex credit budget, for both rate-limit modes); `pickerModes(provider:creditBudget:)` drops the redundant `.sevenDay` tab for a credit budget (`[.fiveHour, .contextHealth]`).
...
| `planType` | `String?` — Codex `plan_type` (free / plus / pro / team / business / …) or `"api"` for API-key accounts; `UsageViewModel.resolveAccountIdentity` syncs it onto `AccountRecord.billingType` (shown as "· Plus" in the picker). nil for Claude. |
...
`parse(openAIHeaders:now:)` reads OpenAI's `x-ratelimit-limit/remaining/reset-requests` and `-tokens` headers (project-token headers ignored); resets are Go durations ("1s", "6m0s", "1h2m3.5s", "20ms") parsed by `parseGoDuration(_:)` and added to `now`. Used for Codex API-key accounts, where these per-minute limits are the only quota signal.
...
| `fiveHourWindowMinutes` / `sevenDayWindowMinutes` | `Int?` — actual window lengths from Codex payloads (300 / 10080); nil for Claude. Consumed via `fiveHourDuration` / `sevenDayDuration` (defaulting to 300 / 10080 minutes when absent), which drive `withClearedRolloverArtifacts` and `estimatedTimeToLimit` |

| `creditBudget` | `CodexCreditBudget?` — set for Codex **spend-control plans** (Business / Enterprise), whose `wham/usage` payload has `rate_limit: null` and reports the budget in `spend_control.individual_limit`. When set, both windows above mirror `usedPercent` / `resetsAt` (so every percent path — menu bar, auto mode, notifications, spike filter — works unchanged) and the popover renders one **Credits** bar. `isCreditBudget` convenience; vocabulary flips to "Credits" / short code `CR`. Threaded through every copy (`markedThrottled`, `withCleared*`, spike display) and persisted (`decodeIfPresent`). |

`CodexCreditBudget` (`Models/CodexCreditBudget.swift`): `usedPercent` (0–100), `used`, `limit`, `remaining`, `unit` ("credit"), `resetsAt`, `reached` (`spend_control.reached`), `hasCredits` / `unlimited` (`credits.*`), `planType`, `balance` (`credits.balance` — purchased credits on subscription plans, spent once the windows are exhausted). `isCreditBudget` is true only when `limit > 0`; a balance-only record (limit 0) keeps the windowed layout and surfaces via `RateLimitUsage.creditBalance` (Weekly-bar `CreditBalanceRow`). `formatCredits(_:)` → "7.0K" / "32.8K" / "950".

**Codex account kinds** (`Models/CodexDisplayKind.swift`): `.windows` (Free/Plus/Pro/Team subscriptions — 5h + weekly windows), `.credits` (Business/Enterprise spend-control budget), `.apiLimits` (API-key mode — pay-per-token, per-minute `x-ratelimit-*` limits only). `CodexDisplayKind.of(rateLimits:standardLimits:apiKeyAccount:)` picks the layout; `MetricMode.shortLabel(provider:kind:)` / `pickerModes(provider:kind:)` pick the vocabulary ("5 Hour | Weekly", "Credits", "API Limits" — the last two collapse to one tab + Context). `CodexAccessMode` (`.chatgpt` / `.apiKey`) is persisted on `AccountRecord.codexAccessMode` (nil = ChatGPT OAuth; `isAPIKeyAccount`). Session logs never carry the budget numbers (`individual_limit` is always null there; only `rate_limit_reached_type` such as `workspace_owner_credits_depleted`), so the budget comes from the endpoint only.

Codex readings are parsed by `CodexUsageParser` (`Models/CodexUsageParser.swift`): `parseUsageResponse(_ data:)` for the `wham/usage` JSON — windowed plans (`rate_limit.primary_window` / `secondary_window` with `used_percent`, `reset_at` / `resets_at`, `limit_window_seconds` / `window_minutes`; `rate_limit_reached_type` → throttled), else spend-control plans via `parseCreditBudget` (numbers accepted as strings or numbers; throttled when `spend_control.reached`, `credits.has_credits == false`, or `rate_limit_reached_type` is non-null) — and `parseSessionRateLimits(_:)` for the `rate_limits` object in a session-log `token_count` event. Both return nil when there is neither a window nor a budget. A 200 with a credits-only body is therefore a `.success` in `CodexRateLimitFetcher`, never an outage/backoff. All hardening below (`markedThrottled`, `withClearedExpiredWindows`, `withClearedRolloverArtifacts`, spike confirmation) is provider-neutral and applies unchanged.
...
`withClearedExpiredWindows(now:)` returns a copy normalizing two kinds of stale state: (1) any window whose `reset` is in the past → utilization `0`, reset `nil`, status `"allowed"` (rolled over); and (2) an **unbounded throttle** — a window with status `"throttled"` but *no* reset → status `"allowed"` (utilization kept). A genuine quota throttle always carries a reset, so a reset-less throttle can never be aged out by (1) and would otherwise stick forever on the stale/cache path; dropping the flag stops the bar from falsely claiming "Throttled". If the binding window is cleared by either rule, `overallStatus` is reset too. Applied at six call sites: cache restore (`RateLimitFetcher.restorePersistedRateLimits`), runtime cache hit (`RateLimitFetcher.cachedOrEmpty`), the snapshot stale-fallback path (`UsageViewModel.effectiveRateLimits`), the FS-event local path (`UsageViewModel.refreshLocalData` — re-publishes the displayed value directly, so without clearing an expired throttle would re-paint on every JSONL burst until the next poll), the offline branch of `refresh()` (a throttle whose reset passes while offline must not alarm until connectivity returns), and the multi-account fan-out (`MultiAccountFanOut.resolve` — a failed per-account fetch falls back to cache, and a cached throttle that expired during an absence must not keep feeding `anyThrottled` in the menu bar). Without these, the menu bar can render `100%` + broken star for hours after a window has reset while the snapshot reuses stale `rateLimits` for up to 24h. A one-shot `rolloverClearTimer` (re-armed on every snapshot publish for the next future reset, via `scheduleRolloverClear`) additionally fires `refreshLocalData()` ~1s after a countdown ends, so an exhausted bar rolls over to 0% the moment the window resets instead of showing a stale 100% until the next poll or JSONL burst.
...
`UsageViewModel.spikeConfirmedRateLimits(fresh:previousDisplayed:previouslyNearFull:now:)` (a `nonisolated static` pure helper in `UsageViewModel+Statics.swift`, returning `SpikeConfirmedRateLimits { display, nearFullWindows, heldWindows }`) covers the case `withClearedRolloverArtifacts` **misses**: a window that reads near-full **more than 10 min** into its cycle. The `/api/oauth/usage` endpoint can return a transport-**fresh** but wrong ~100% reading (server-side eventual consistency after wake or around other discontinuities), which the `rateLimitsFresh` gate would otherwise trust and paint as "Limit reached". Per window, if utilization ≥ `rolloverArtifactUtilizationThreshold` (0.95) **and** the status is not an explicit `"throttled"` **and** the *overall* status is not `"throttled"` pointing at this (binding) window — `isThrottled` treats an overall-only throttle as real even when the per-window status lags behind as `"allowed"`, so holding it would hide an authoritative throttle — **and** there is a `previousDisplayed` value at all **and** the window's current near-full spike sequence is not yet **confirmed** (below), the window is *held*: its displayed **utilization** is substituted from `previousDisplayed` (the fresh reset is kept; the held **status is always `"allowed"`** — never inherited from the previous poll, so a hold can't re-display a throttle the server just cleared; the display's `overallStatus` is derived from its own window statuses when the binding window is held, so a hold never masks a genuine throttle on the *other* window), so a spike shows the last-known-good percentage — "rather show stale than a false used value". **Every** window at/above the threshold — *including a genuinely throttled one* — is recorded in `nearFullWindows` as a `NearFullMemory { reset, firstSeen, confirmed }` **keyed by that reading's reset instant** (reset-less readings use a `distantPast` sentinel), which `refresh()` persists to `previouslyNearFullWindows: [String: NearFullMemory]` and passes back next poll. A remembered near-full vouches only for the **same window instance**: the stored reset must match the fresh reading's reset within `nearFullResetMatchTolerance` (60s) — so a memory from before a rollover (e.g. an endpoint outage spanning the reset with no wake/unlock to clear it) can never vouch for the new window; on a reset mismatch the sequence restarts with a new `firstSeen`. **Confirmation is time-based, not poll-count-based** (changed 2026-08-11 after a false 7-day 100% — account actually at 2% — persisted across multiple consecutive polls and sailed through the old two-poll confirmation): an unconfirmed spike is accepted only once its consecutive near-full sequence has aged past `spikeConfirmationMinimumAge` (600s; `firstSeen` is preserved across held polls so a persistent spike ages into confirmation). A memory is created `confirmed: true` whenever its reading was *displayed as fresh* (authoritative throttle bypass, accepted cold-start, or an age-confirmed sequence), and a confirmed memory keeps subsequent polls of the same window instance accepted without re-holding — so the poll right after a throttle clears (still ~96%, now `"allowed"`) reads as a confirmed continuation rather than an isolated spike. A low reading (below threshold) drops the window's memory: the next spike starts a new sequence. Because substitution uses the previous *displayed* value, a genuinely sustained limit (previous was also near-full) stays near-full — a real limit is never hidden. A genuine `throttled` status bypasses the filter entirely (authoritative, shown immediately). With no `previousDisplayed` (true cold start, no cache) **nothing is held** — the fresh reading is the only real value there is, and fabricating a 0% would be a false *low* (the alarm stays gated per-window regardless). `previouslyNearFullWindows` is cleared on any return-from-absence (`resumeTimers` — wake / unlock / idle-resume) and on `switchAccount`, so a pre-absence near-full reading can't auto-confirm a post-absence glitch. Only runs on genuinely fresh readings; header-less / cached polls keep falling back to the held value + local estimate. Holds emit an `AppLogger.network.notice` with the raw → held values. The spike-confirmed value is what feeds aggregation, notifications (`alertableRateLimits`), and the fan-out seed. On a hold, `refresh()` also calls `RateLimitFetcher.overrideCachedRateLimits(_:accountId:defaults:)` — `fetch()` already cached+persisted the raw glitch value before the filter ran, and the write-back replaces it (preserving profile/source/standardLimits/fetchedAt; no-op when no cached entry exists) so the glitch can't survive as the stale fallback that a later instant-paint (wake / cold start / launch restore) would re-display, or be seeded as `previousDisplayed` where a repeat glitch could confirm it. The write-back only runs when **every held window substituted a real (non-zero) prior value** — a hold at a zeroed baseline (window just reset) must not overwrite the real server reading on disk with a fabricated 0% that would survive relaunch as "recent" data. **Known limitation:** non-active accounts in the multi-account fan-out have no per-account near-full memory, so a spike on a *secondary* account's slot is only guarded by `withClearedExpiredWindows` + `withClearedRolloverArtifacts`, not this confirmation filter. **Design trade-off (observed live 2026-07-04, interval widened 2026-08-11):** the `/api/oauth/usage` endpoint reports a genuine hard limit as near-full utilization with status `"allowed"` (it does not flip to `"throttled"`), so a real limit crossing is held too — "Limit reached" appears up to `spikeConfirmationMinimumAge` (~10 min) after the server first reports it, on top of the server's own lag. That is the accepted cost of never alarming falsely on a multi-poll server glitch.
...
- **Codex API-key accounts** (`Services/OAuthManager+Codex.swift`): `registerCodexAPIKey(_:) -> Result<Void, AuthError>` validates `sk-…`, stores the key in the Keychain as the account's refresh token (served as the access token, `expiresAt = .distantFuture`; `refreshCodexAccessToken` short-circuits for `isAPIKeyAccount` so the token endpoint is never hit), and adds an `AccountRecord(provider: .codex, codexAccessMode: .apiKey, billingType: "api")`. Account id = `apiKeyAccountId(for:)` = `"openai-api-" + SHA-256(key)[0..<24]` — the key never lands in UserDefaults. `CodexAuthFileImporter.parseCredential` accepts both `auth_mode` values (`.chatGPT(CodexImportedAuth)` / `.apiKey(String)`), so "Import Codex CLI login" seeds either kind.
...
- Computed: `activeAccount`, `accounts(for: AIProvider)`, `canAddAccount(provider:)` (< 3 for that provider), `account(id:)`, `provider(of:)` (`.claude` for unknown ids — the single home for that lookup; there is no any-provider cap check)
...
- **`overrideCachedRateLimits(_:accountId:defaults:)`** replaces an account's cached + persisted rate limits with a corrected value, preserving all other fields. Called from `UsageViewModel.refresh()` when the spike filter holds an unconfirmed near-full reading (see `spikeConfirmedRateLimits`), because `fetch()` persists the raw server value before the ViewModel can correct it. No-op when the account has no cached entry.
...
- `parseStatus(_ summary: StatusPageSummary, config:)` — `nonisolated static`, pure. Without a filter it uses ALL API-returned components for the worst-status calculation. With a filter, only filtered components populate `componentStatuses` / drive the worst indicator / description, and an active incident counts only when its `components` list explicitly names a filtered component (an incident with no component list is ignored — it could be about anything else on the page). A filtered feed whose components are all absent from the summary reports `.unknown` rather than the whole page's unrelated indicator. The Statuspage JSON models (`StatusPageSummary`, `StatusPageComponent`, `StatusPageIncident` — the latter now with optional `components`) are internal so tests build summaries directly.
- Legacy statics kept for callers/tests: `StatusChecker.statusPageBaseURL` (Claude page) and `StatusChecker.knownComponents` (Claude catalog). The Codex feed config lives in `Services/StatusFeedConfig+Codex.swift`.
...
- **API-key accounts**: `fetchAPIKeyLimits(apiKey:accountId:)` — `POST https://api.openai.com/v1/responses` (`{"model","input":".","max_output_tokens":16}`, `Authorization: Bearer <key>`), trying `gpt-5-nano` → `gpt-5-mini` → `gpt-5` on a 400/404 model-not-found. `interpretAPIKeyProbe(statusCode:headers:now:)` (pure): 401/403 → `.authFailed`; any < 500 response carrying `x-ratelimit-*` headers (2xx, 429, 400) → `.success` with `standardLimits` + `hasStandardRateLimitHeaders` + `planType: "api"`; else `.unavailable` (same backoff). No session-log fallback. Cost: 16 nano output tokens per poll. The persisted blob (`PersistedCodexRateLimits`) also carries `standardLimits` and `planType`, and `cachedOrEmpty` passes them through.
...
- `checkRateLimitAlerts(rateLimits:)` labels windows via `windowLabels(for: rateLimits)` → `("5-Hour", "7-Day")` for Claude, `("5-Hour", "Weekly")` for Codex windowed plans, `("Credits", "Credits")` for a Codex credit budget. Dedup keys (`rateLimit5h`, `rateLimit7d`) are provider-neutral.
...
- **Caller-side stale-data gate**: `UsageViewModel` only fires a notification on genuinely fresh, spike-confirmed rate limits, via `UsageViewModel.alertableRateLimits(confirmed:rateLimitsFresh:)` — returns nil unless `rateLimitsFresh` is true, and takes the **spike-confirmed** limits (`spikeConfirmedRateLimits`) rather than the raw fetch, so a held/unconfirmed near-full spike can't fire a false alert. The menu bar / popover *alarm* uses the same freshness + `alarmConfirmed(rateLimitsFresh:displayedIsThrottled:)` gate, which suppresses a successful-but-header-less fetch that reuses held stale limits
...
- `static configure(_:)` — the one place update-check policy is set; both inits route through it
- Testable `init(controller:)` applies `configure(_:)` but **never** calls `startUpdater()`. Tests must use it instead of `.shared`: starting a real updater inside the test runner makes Sparkle's standard user driver raise a **modal NSAlert** when host-bundle validation / the update check fails there, freezing the main thread (and every `@MainActor` test) until dismissed — the intermittent full-suite hang on loaded local runs and the 19-min-timeout kills on the headless CI runner (diagnosed 2026-08-11 via `sample`: main thread parked in `-[NSAlert runModal]` under Sparkle)
...
- **Provider routing** lives in `ViewModels/UsageViewModel+ProviderRouting.swift` (`aggregator(for:)`, `provider(ofAccount:)`, `cachedResult(for:accountId:)`, `overrideCachedRateLimits(_:for:accountId:)`, `fetchAPIData`, `resolveAccountIdentity`). `fetchAPIData` routes Codex API-key accounts to `CodexRateLimitFetcher.fetchAPIKeyLimits`; `aggregateOffMain` stamps `UsageSnapshot.costIsBilled` (true for API-key accounts → cost rows drop the "~" and are a real bill at API rates); `resolveAccountIdentity` syncs a fresh `planType` onto the Codex record's `billingType`.
...
- Deinit: invalidates polling timer + rollover-clear timer, removes sleep/wake observers (FileWatcher's own deinit handles its cleanup)
...
- **Sleep/wake lifecycle**: `setupSleepWakeObservers()` subscribes to `NSWorkspace.willSleepNotification` (suspends timers) and `didWakeNotification` (resumes timers, triggers immediate refresh). Also subscribes to `sessionDidResignActiveNotification` (screen lock → suspend) and `sessionDidBecomeActiveNotification` (unlock → resume + refresh). **Every return-from-absence path** (wake, unlock, idle-resume via activity monitor, and `resumeFromUserInteraction` when actually suspended) first calls the shared `repaintCachedNotFresh()` helper: repaint from the persisted cache (`cachedOrEmpty` → expired-window clearing → `withClearedRolloverArtifacts`) with `rateLimitsFresh` dropped — the pre-absence snapshot may be hours old yet still flagged fresh, which would keep the alarm armed until the post-absence fetch lands (a window can reset during a long lock). The freshness flag drops even when the cache is empty; an ordinary menu-bar click while active does NOT repaint (that would gate a legitimately armed alarm on every popover open).
