...
- **Pure response interpreters**: `interpretUsageEndpoint(statusCode:data:headers:cachedProfile:) -> APIFetchResult?` and `interpretClaudeCodeClientData(statusCode:data:headers:cachedProfile:callerStandardLimits:) -> APIFetchResult?` are `nonisolated static` pure functions that own the status-code / payload / 429-throttle contract for both OAuth endpoints. The async `fetchUsageEndpoint` / `fetchClaudeCodeClientData` wrappers do the HTTP call + diagnostic logging and delegate interpretation. Allows contract testing of every payload shape and status-code combination without mocking URLSession.
