...
`UsageViewModel.spikeConfirmedRateLimits(fresh:previousDisplayed:previouslyNearFull:)` (a `nonisolated static` pure helper in `UsageViewModel+Statics.swift`, returning `SpikeConfirmedRateLimits { display, nearFullWindows, heldWindows }`) covers the case `withClearedRolloverArtifacts` **misses**: a window that reads near-full **more than 10 min** into its cycle. The `/api/oauth/usage` endpoint can return a transport-**fresh** but wrong ~100% for a window right after wake (server-side eventual consistency), which the `rateLimitsFresh` gate would otherwise trust and paint as "Limit reached" before the next poll corrects it. Per window, if utilization ≥ `rolloverArtifactUtilizationThreshold` (0.95) **and** the status is not an explicit `"throttled"` **and** the *overall* status is not `"throttled"` pointing at this (binding) window — `isThrottled` treats an overall-only throttle as real even when the per-window status lags behind as `"allowed"`, so holding it would hide an authoritative throttle — **and** there is a `previousDisplayed` value at all **and** the window was **not** near-full on the previous fresh poll (`previouslyNearFull`), the window is *held*: its displayed **utilization** is substituted from `previousDisplayed` (the fresh reset is kept; the held **status is always `"allowed"`** — never inherited from the previous poll, so a hold can't re-display a throttle the server just cleared; the display's `overallStatus` is derived from its own window statuses when the binding window is held, so a hold never masks a genuine throttle on the *other* window), so a one-off spike shows the last-known-good percentage — "rather show stale than a false used value". **Every** window at/above the threshold — *including a genuinely throttled one* — is recorded in `nearFullWindows` **keyed by that reading's reset instant** (reset-less readings use a `distantPast` sentinel), which `refresh()` persists to `previouslyNearFullWindows: [String: Date]` and passes back next poll. A remembered near-full confirms only the **same window instance**: the stored reset must match the fresh reading's reset within `nearFullResetMatchTolerance` (60s) — so a memory from before a rollover (e.g. an endpoint outage spanning the reset with no wake/unlock to clear it) can never vouch for the new window. A **second consecutive** matching fresh near-full reading is trusted (two-poll confirmation), and the poll right after a throttle clears (still ~96%, now `"allowed"`) reads as a confirmed continuation rather than an isolated spike. Because substitution uses the previous *displayed* value, a genuinely sustained limit (previous was also near-full) stays near-full — a real limit is never hidden, only delayed by at most one poll. A genuine `throttled` status bypasses the filter entirely (authoritative, shown immediately). With no `previousDisplayed` (true cold start, no cache) **nothing is held** — the fresh reading is the only real value there is, and fabricating a 0% would be a false *low* (the alarm stays gated per-window regardless). `previouslyNearFullWindows` is cleared on any return-from-absence (`resumeTimers` — wake / unlock / idle-resume) and on `switchAccount`, so a pre-absence near-full reading can't auto-confirm a post-absence glitch. Only runs on genuinely fresh readings; header-less / cached polls keep falling back to the held value + local estimate. Holds emit an `AppLogger.network.notice` with the raw → held values. The spike-confirmed value is what feeds aggregation, notifications (`alertableRateLimits`), and the fan-out seed. On a hold, `refresh()` also calls `RateLimitFetcher.overrideCachedRateLimits(_:accountId:defaults:)` — `fetch()` already cached+persisted the raw glitch value before the filter ran, and the write-back replaces it (preserving profile/source/standardLimits/fetchedAt; no-op when no cached entry exists) so the glitch can't survive as the stale fallback that a later instant-paint (wake / cold start / launch restore) would re-display, or be seeded as `previousDisplayed` where a repeat glitch could confirm it. The write-back only runs when **every held window substituted a real (non-zero) prior value** — a hold at a zeroed baseline (window just reset) must not overwrite the real server reading on disk with a fabricated 0% that would survive relaunch as "recent" data. **Known limitation:** non-active accounts in the multi-account fan-out have no per-account near-full memory, so a spike on a *secondary* account's slot is only guarded by `withClearedExpiredWindows` + `withClearedRolloverArtifacts`, not this two-poll confirmation. **Design trade-off (observed live 2026-07-04):** the `/api/oauth/usage` endpoint reports a genuine hard limit as near-full utilization with status `"allowed"` (it does not flip to `"throttled"`), so a real limit crossing takes the one-poll hold too — "Limit reached" appears one poll (~30–120s) after the server first reports it, on top of the server's own lag. That is the accepted cost of never alarming falsely.
