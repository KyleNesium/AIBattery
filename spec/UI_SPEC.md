...
Popover animations require user interaction (`withAnimation` blocks) or view-lifecycle events, so a hidden panel triggers none of them. **However, `orderOut` does NOT remove the hosting view from the hierarchy** — `.onDisappear` never fires, and the hidden SwiftUI tree keeps re-evaluating (body + layout passes) on every `@Published` change; a 12s `sample` profile (2026-07-01) caught `StackLayout`/`sizeThatFits`/`NSHostingView.layout` churn with the panel closed. Mitigation: the heavy sections (Projects/Insights incl. Swift Charts) are gated on `DeferredRenderState.hasAppeared`, which is **disarmed on `.panelDidDismiss`** (via `resetTransientPopoverState`) and re-armed one run-loop after `.panelDidShow` (every real open posts it; `onAppear` deliberately does NOT arm — it fires at setup when the hosting view attaches, and would race the launch pre-warm's `orderOut`, potentially leaving the heavy sections armed on a never-opened panel) — so the hidden tree collapses to the cheap core sections. `MarqueeText` explicitly calls `cancelAndStop()` in `.onDisappear`. The only AppKit-layer timers (`StatusBarManager` countdown ticker and the one-shot 30s sparkle end-timer) are not popover animations.
...
**Stale alarm suppression** (unconfirmed data): a stale *number* is fine, but a stale *alarm* is a false alarm. The alarm gate is **not** `!isShowingCachedData` — that flag only reports whether the network *fetch* hit cache, and a fetch can succeed (not cached) while carrying **no** unified rate-limit headers (~90% of polls), in which case the held stale rate limits are reused for up to 24h (`rateLimitStaleTTL`). The gate is instead `UsageViewModel.alarmConfirmed(rateLimitsFresh:displayedIsThrottled:)` = **`rateLimitsFresh || displayed-data-is-throttled`**: `rateLimitsFresh` (a `@Published` set in `refresh()`) is true only when this cycle returned unified headers AND wasn't cache-served; a genuine throttle is admitted even when held, because it is authoritative and self-expires via `withClearedExpiredWindows`. `StatusBarManager` passes this `confirmed` into `MenuBarMultiAccountText.resolveDisplay`, which on `false` suppresses the throttle alarm: **no broken star and no "limit reached" countdown**. The throttle-transition tracking that drives the recovery sparkle keys off this same gate. Asymmetry rationale: a *bare* ≥100% requires fresh data (a stale held 100% can't arm it); a genuine throttle persists across the header-less polls without flickering. Without this, waking the machine showed a false "Limit reached" until a fresh-headers poll landed (which could be many minutes on a 5-min interval).

**Displayed percentage is always the real API value** (never a local estimate): the freshness gate suppresses the *alarm*, not the *number*. `UsageSnapshot.percent(for:)` shows `rateLimits.fiveHourPercent`/`sevenDayPercent` — fresh, or the last-known held value on a header-less-but-successful poll (which is *not* a connection error, so we still show real data). A token-derived `LocalUsageEstimate` is **not** substituted for a held API reading; it is used only when there is no API rate-limit data at all (standard-API-key users, via `LocalEstimateSection`). A fresh-but-wrong near-full reading never reaches the bar as a high number because it is corrected upstream (see **Confirm-before-alarming** below), so a stale number never needs a local-estimate "correction".

The **popover's 5-Hour/7-Day bars** read from a single source of truth — the (already-corrected) snapshot rate limits: `UsagePopoverView` passes `confirmed: snapshot.rateLimitPercentConfirmed(for:)` (the **per-window** alarm gate — a throttle on one window can't confirm the other's stale reading) into `FiveHourBarSection`/`SevenDayBarSection` → `UsageBar`, which shows `limits.fiveHourPercent`/`sevenDayPercent` and derives a pure `UsageBar.AlarmState(percent:isThrottled:confirmed:)`. On unconfirmed data the alarm is suppressed — **no "Throttled" text, no warning triangle, no "Limit reached", and no burn-rate "~X to limit" estimate** (a prediction off the API utilization, gated on `confirmed` too). `throttled` and `limitReached` are mutually exclusive flags (Throttled wins, matching the footer's if/else-if order). Reset countdown and the `binding` badge read from the held `rateLimits`. Confirmed data behaves exactly as before. Without this gate the popover showed "5-Hour 100% Limit reached" on a stale reading whose window was actually at ~7% (the wake bug); the v2.5.0/2.5.1 fix gated only the menu bar and the network-cache flag, missing the header-less-but-fresh fetch path.

**Confirm-before-alarming** (fresh-but-wrong spike): the gates above defend against *stale/held* data, but the endpoint can also return a **transport-fresh yet wrong** ~100% for a window right after wake (server-side eventual consistency), on a window that is actually low. That reading is genuinely fresh (`rateLimitsFresh == true`), so all the gates above trust it — and for a poll or two the bar flips to a red "5-Hour 100% Limit reached" before the next fetch corrects it to the real value (e.g. 2%). `refresh()` now runs a fresh reading through `spikeConfirmedRateLimits` (see DATA_LAYER.md): a bare near-full spike (≥95%, non-throttled) that wasn't near-full on the previous fresh poll is **held at the previous displayed value** until a second consecutive fresh poll confirms it — so the bar shows the last-known-good percentage, not the false maxed reading ("rather show stale than a false used value"). A genuine throttle is authoritative and shown immediately; a genuinely sustained limit is confirmed one poll later. Because the *value* itself is corrected before it reaches the snapshot, every downstream surface agrees — the popover bars, the menu-bar star/percent, notifications, and the multi-account seed all see the held value, so none of them flash a false limit. The near-full memory resets on wake / unlock / idle-resume / account switch so a pre-absence near-full reading can't auto-confirm a post-absence glitch.
...
- Lifecycle notifications: `.panelDidDismiss` posted from the consolidated `onDismiss` (every orderOut path) — SwiftUI resets transient state AND collapses the deferred heavy sections; `.panelDidShow` posted after `orderFrontRegardless` — re-arms them (needed because the view never leaves the hierarchy, so `onAppear` fires only once per launch). Also posted when `isAuthenticated` flips back to true while the panel is showing — signing out swaps `UsagePopoverView` for `AuthView` (structural removal fires `onDisappear` → disarm), and signing back in re-inserts it without a panel close/reopen, so no click would otherwise re-arm. That post runs **after** `viewModel.refresh()` completes, not before: the sink fires before SwiftUI commits the structural swap, and a notification posted while the re-inserted view's `.onReceive` subscription doesn't exist yet would be silently dropped — the refresh's awaits give SwiftUI the run-loop cycles to attach the subscriber first.
