...
The **popover's 5-Hour/7-Day bars apply the same gate**: `UsagePopoverView` passes `confirmed: !viewModel.isShowingCachedData` into `FiveHourBarSection`/`SevenDayBarSection` → `UsageBar`, which derives a pure `UsageBar.AlarmState(percent:isThrottled:confirmed:)`. On unconfirmed data the alarm is suppressed — **no "Throttled" text, no warning triangle, no "Limit reached", and the display percent is *not* clamped to 100** (it shows the real last-known percent). `throttled` and `limitReached` are mutually exclusive flags (Throttled wins, matching the footer's if/else-if order). Confirmed data behaves exactly as before. Without this gate the popover could show "7-Day 100% Limit reached" on a cached reading while the menu bar (already gated) showed a healthy green star — the v2.5.0 symptom.
