...
**Stale percentage correction**: beyond suppressing the alarm, the *displayed percentage* is also corrected when unconfirmed. `UsageSnapshot.percent(for:)` delegates to the pure `resolvedPercent(api:local:confirmed:)`: when **not** confirmed it prefers the local token estimate (`LocalUsageEstimate`) over the held API utilization, falling back to the API value only if the account isn't calibrated. So a stale held 100% on wake reads as the real low % (e.g. 7%) — green bar, green star — instead of a maxed-out red bar. The snapshot carries `rateLimitPercentConfirmed` (set by `refresh()` to the same `alarmConfirmed` value) so the menu bar (`percent(for:)`) and popover bars agree.

The **popover's 5-Hour/7-Day bars apply both** from a single source of truth — the snapshot: `UsagePopoverView` passes `displayPercent: snapshot.percent(for: .fiveHour/.sevenDay)` (the corrected %) and `confirmed: snapshot.rateLimitPercentConfirmed(for:)` (the same **per-window** flag `percent(for:)` keys off, so the displayed percent and the alarm gate can't diverge, and a throttle on one window can't confirm the other's stale reading) into `FiveHourBarSection`/`SevenDayBarSection` → `UsageBar`, which derives a pure `UsageBar.AlarmState(percent:isThrottled:confirmed:)`. On unconfirmed data the alarm is suppressed — **no "Throttled" text, no warning triangle, no "Limit reached", and no burn-rate "~X to limit" estimate** (it's a prediction off the API utilization, so it's gated on `confirmed` too — otherwise a stale 100% reading shows "~0s to limit" next to the corrected low %) — and the corrected percent (local estimate) is shown. `throttled` and `limitReached` are mutually exclusive flags (Throttled wins, matching the footer's if/else-if order). Reset countdown and the `binding` badge still read from the held `rateLimits`. Confirmed data behaves exactly as before. Without these gates the popover showed "5-Hour 100% Limit reached" on a stale reading whose window was actually at ~7% (the wake bug); the v2.5.0/2.5.1 fix gated only the menu bar and the network-cache flag, missing the header-less-but-fresh fetch path.
