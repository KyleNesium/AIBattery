...
macOS menu bar app showing Codex and Codex (OpenAI) usage at a glance — up to 3 accounts per provider. Built with Swift 6 / SwiftUI, SPM, macOS 13+.
...
1232 tests across 95 files, using `import Testing` + `@testable import AIBatteryCore`.
...
- Codex local data comes from `~/.codex/sessions` rollouts via `CodexSessionLogParser` → the **same** `AssistantUsageEntry`, so `UsageAggregator` is shared (one instance per provider, `Codex-`/`gpt-` model filter). Only `session_meta`, `turn_context` and `token_count.last_token_usage` are decoded — `response_item` (message content) is rejected on `type`. Codex has no stats cache: all-time figures are bounded by rollout retention and the UI says so.
- Codex 5-hour / 7-day usage may come from Codex client metadata rather than public `/v1/messages` headers
...
- StatusChecker backs off exponentially after failures (60s → 120s → 240s, capped at 5 min, ±20% jitter) — no immediate retries. One `StatusChecker` per feed (`.shared` Codex, `.codex` OpenAI filtered to the five Codex components). `CodexRateLimitFetcher` applies the same backoff curve per account to the `wham/usage` endpoint and serves the session-log fallback meanwhile
...
- OAuth refresh token lives in macOS Keychain under service `"AIBattery"` (`refreshToken_<id>` Codex, `refreshToken_codex_<id>` Codex) — access token is memory-only (re-derived from refresh on launch), expiry timestamp in UserDefaults. Only 1 Keychain item per account to minimize Sparkle update prompts.
