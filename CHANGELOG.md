...
- **Usage projections** — trend arrow (↑/↓/→) comparing this week vs last, projected daily total, busiest day of the week
...
- **Menu bar decimal precision** — optional one-decimal display (`42.5%` instead of `42%`)
- **Compact rate limit mode** — non-selected windows collapse to `"5h: XX% · 7d: XX%"` single-line summary
...
- New files: `ThemeColors.swift`, `SettingsManager.swift`, `TutorialOverlay.swift`
- New tests: `ThemeColorsTests`, `SettingsManagerTests`, extended `RateLimitUsageTests`, `UsageSnapshotTests`, `TokenHealthMonitorTests`
...
- **Click-to-copy** on stat values (percentages, tokens, costs) with checkmark feedback
...
### Improved
- Extended test coverage and optimized hot paths
- Renamed "Cost" toggle to "Cost~" for compact display row
