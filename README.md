...
**1005 tests** across 67 test files.

| Area | Tests | What's covered |
|------|-------|----------------|
| Models | 257 | Token summaries, rate limit parsing (unified + standard + client data edge cases, timestamp rejection, 100%-without-status is not throttled), health status, metric modes, API profiles, usage snapshots (incl. escalation ladder, hysteresis), model pricing, Claude system status indicators, standard rate limits, local usage estimate calibration policy (uncalibrated-only seeding), expired-window clearing on cache restore |
