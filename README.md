...
| ViewModels | 63 | Refresh interval clamping, error messages (incl. standard limits fallback), adaptive polling, throttle tracking, idle threshold constants, TTL-based stale rate limit expiry (incl. expired-window clearing on stale fallback), effective value generic guard, auth-error reconnect prompt, cross-account refresh-race guard (results discarded when the active account changes mid-fetch), stale-data notification gate (cached rate limits never fire alerts), multi-account fan-out orchestration (end-to-end through the concurrent TaskGroup: toggle-off clearing, seed dedup so the active account isn't re-fetched, missing-token skip, seed-only path) + shared eligible-account filter (non-pending AND authenticated, store order) |
