...
| Step | Claude | Codex |
|:---:|---|---|
| **1** | Launch AI Battery — the auth screen appears on first run (use "Sign in with Codex instead" to switch) | same |
...
**Multiple accounts:** Use the header dropdown to switch accounts or add new ones (**Add Claude Account…** / **Add Codex Account…**). Each account has its own rate limits, tokens, and identity; with both providers connected the picker and menu bar prefix each account with its glyph (✦ Claude, ⬡ Codex).
...
|---|---|
| ➕ **Add Account** | Connect another Claude account (up to 3 per provider; Codex accounts are added from the header picker) |
